import {
  quotaDecisionKindSchema,
  quotaPolicySchema,
  quotaWindowTypeSchema,
  type QuotaDecisionKind,
  type QuotaPolicy,
  type QuotaWindowType,
} from "@lingban/contracts";

export function resolveQuotaWindowRange(windowType: QuotaWindowType, at: string | Date) {
...
export function evaluateQuotaPolicyValue(policy: QuotaPolicy, currentValue: number): {
  ...
  if (hardLimit != null && normalizedValue > hardLimit) {
    const decision =
      parsedPolicy.actionOnHardLimit === "require_override" ? "require_approval" : "block";
    return {
  ...
  if (softLimit != null && normalizedValue > softLimit) {
    const decision =
      parsedPolicy.actionOnSoftLimit === "require_approval" ? "require_approval" : "warn";
    return {
      decision: quotaDecisionKindSchema.parse(decision),
...
export function quotaDecisionSeverity(decision: QuotaDecisionKind) {
