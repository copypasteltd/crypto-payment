...
test("workspace governance smoke: postgres-backed members and invitations", async () => {
  const scriptPath = path.resolve("tests/workspace-membership-invitations.scenario.mjs");

  const result = await new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [scriptPath, "--storage=postgres"], {
      cwd: path.resolve("."),
  ...
  assert.equal(
    result.code,
    0,
    `postgres workspace governance scenario failed (exit=${result.code}, signal=${result.signal ?? "null"})\n${result.stderr || result.stdout}`
  );
