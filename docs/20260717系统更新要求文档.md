...
| 双端职责清晰 | Dashboard 承担生产和治理，H5 承担低门槛消费与移动协作 |
...
Session 项目创建、脱敏规则、Replay、Seal、Package、工坊创建和发布通过 Dashboard Creator 完成。H5提供明确的 Dashboard 深链入口。
...
| 决策项 | 冻结要求 |
|---|---|
| 供给起点 | Dashboard Creator 创建 Session Project 和空白 Source Run |
...
| `app/mobile` | Taro H5 与后续小程序 | 新消费启动链、多任务对话、移动 Source Run 协作 |
...
H5任务列表允许显示有权限访问的 Source Run，使用“录制实例”业务文案。H5不展示 Pack、Revision、Seal 等专业操作。用户点击“管理 Session 资产”时跳转 Dashboard 深链。
