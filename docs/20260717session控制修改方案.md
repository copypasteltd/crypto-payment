...
| SC-034 | 完成 | 本地无虚拟化回归通过；HZ01 新版本进程、App Server 配置、Migration 0030、readiness 和公网入口已验收 |
...
| 项目 | 验收结果 |
|---|---|
| 发布节点 | HZ01，`192.168.31.20` |
| 当前发布目录 | `/srv/lingban/releases/20260716T215737Z` |
| API 进程 | `active`，进程路径指向当前发布目录 |
| Run Worker 进程 | `active`，进程路径指向当前发布目录 |
| Nginx | `active` |
| 管理面版本标识 | `LINGBAN_RELEASE=20260716T215737Z`，与当前发布目录一致 |
| Migration | `0030_session_control` 已应用，待执行数量为 0 |
| Session 数据表 | 12 张 Session 资产表可查询 |
| API Readiness | `ready`，Database、Object Storage、Bridge Registry、Runtime、Worker Ops 均通过 |
| Worker Readiness | `ready`，Queue、Worker、DLQ、Queue Events 和 Runtime Backend 均通过 |
| Codex 协议 | `CODEX_RUNTIME_PROTOCOL=app-server` |
| 签名密封 | HMAC-SHA256 已启用；Key ID 固定；Secret 长度 64；环境文件权限 `0600` |
| Provider 数据 | 原有唯一 Provider `prv_00000005 / sub2` 保留且启用 |
| 服务日志 | 新版本启动后无 warning/error 日志 |

### 49.2 功能开关证据

`/internal/metrics` 返回以下值：

```text
session_control_feature_enabled{feature="capture_v2"} 1
session_control_feature_enabled{feature="pack_v2_write"} 1
session_control_feature_enabled{feature="version_immutability"} 1
session_control_feature_enabled{feature="explicit_binding"} 1
```

Session Capture、Draft、Version、Legacy Migration 路由均已注册。未携带登录凭证时返回 `401`，说明路由存在且鉴权生效。

### 49.3 可访问地址

| 端 | 地址 | HTTP 验收 |
|---|---|---|
| Workspace Dashboard | `http://192.168.31.20:38110/workspace/workshops` | 200 |
| Mobile H5 | `http://192.168.31.20:38120/` | 200 |
| Public API | `http://192.168.31.20:38130/health` | 200 |
| Platform Admin | `http://192.168.31.20:38140/` | 200 |

### 49.4 发布过程修正

首次安装验收发现旧版安装器通过 `systemctl list-unit-files | grep` 判断 unit，软链接切换后没有触发服务重启。安装器已改为 `systemctl cat <unit>` 检测，并在切换后重启 API 与 Run Worker。HZ01 已显式重启，两个进程均指向当前发布目录。

### 49.5 验收边界

本次完成单节点 HZ01 发布和功能基线验收。全量生产 Go-Live 仍执行第 38.3 节的 7 天灰度、告警、备份、容量和多节点验证门禁。
