...
function truncateTextPreview(content) {
...
function isWordPreviewMimeType(mimeType) {
...
async function extractPresentationPreview(buffer) {
...
export function resolvePathWithinRoot(rootPath, requestedPath) {
...
export function createLogicalPath(rootPath, absolutePath) {
...
export function normalizeLogicalPrefix(value) {
    if (!value?.trim()) {
        return null;
    }
...
export function guessFileMimeType(filePath, explicitMimeType) {
...
export function isOfficePreviewMimeType(mimeType) {
    const normalized = normalizeMimeType(mimeType);
    return (isWordPreviewMimeType(normalized) ||
        isSpreadsheetPreviewMimeType(normalized) ||
        isPresentationPreviewMimeType(normalized));
}
export function inferFilePreviewMode(filePath, mimeType) {
    const normalizedMimeType = normalizeMimeType(mimeType) ?? normalizeMimeType(guessFileMimeType(filePath));
    if (normalizedMimeType?.startsWith("text/")) {
        return "text";
    }
    if (normalizedMimeType === "application/json; charset=utf-8" ||
...
export async function readTextPreviewFromPath(absolutePath) {
...
export async function readTextPreviewFromStream(stream) {
    const chunks = [];
    let sizeBytes = 0;
    try {
        for await (const chunk of stream) {
...
export async function extractOfficePreviewFromPath(absolutePath, mimeType) {
    const buffer = await fs.readFile(absolutePath);
    return extractOfficePreviewFromBuffer(buffer, mimeType);
}
export async function extractOfficePreviewFromStream(stream, mimeType) {
    const buffer = await readBufferFromStream(stream);
    return extractOfficePreviewFromBuffer(buffer, mimeType);
}
export async function extractOfficePreviewFromBuffer(buffer, mimeType) {
