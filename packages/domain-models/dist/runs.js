import { bridgeEventSchema, createRunInputSchema, runInformationCollectionAnswerSchema, runInformationCollectionSchema, runInformationCollectionSlotSchema, listRunsQuerySchema, runApprovalSchema, runAttentionModeSchema, runArtifactSchema, runListSummarySchema, runListViewStatusSchema, runRecordSchema, runSnapshotSchema, runStatusSchema, } from "@lingban/contracts";
...
export function resolveRunListViewStatus(status) {
...
function hasRunOutput(snapshot) {
    if (snapshot.run.status === "SUCCEEDED") {
        return true;
    }
    return snapshot.files.some((file) => !file.path.endsWith("/") &&
        (file.kind === "output" || file.kind === "archive" || file.kind === "receipt"));
}
export function resolveRunAttentionMode(snapshot) {
...
function inferRunDomainTag(snapshot) {
    const haystack = [
...
export function resolveRunListTags(snapshot, options = {}) {
    const current = runSnapshotSchema.parse(snapshot);
    const tags = new Set();
    const workspaceContextKey = options.workspaceContextKey?.trim() ||
...
export function matchesRunListQuery(snapshot, query = {}, options = {}) {
    const current = runSnapshotSchema.parse(snapshot);
...
function sortFacetCounts(items) {
...
export function summarizeRunSnapshots(snapshots, options = {}) {
    ...
    for (const snapshot of current) {
        ...
        byEntrySurface.set(snapshot.run.entrySurface, (byEntrySurface.get(snapshot.run.entrySurface) ?? 0) + 1);
    ...
    return runListSummarySchema.parse({
        ...
        byViewStatus,
        byAttentionMode,
        byEntrySurface: sortFacetCounts([...byEntrySurface.entries()].map(([key, count]) => ({ key, count }))),
        byTag: sortFacetCounts([...byTag.entries()].map(([key, count]) => ({ key, count }))),
...
export function transitionRunStatus(run, nextStatus, options) {
...
function projectRunStatus(run, nextStatus, options) {
...
export function applyBridgeEventToRunSnapshot(snapshot, event) {
    const current = runSnapshotSchema.parse(snapshot);
    ...
    switch (parsed.type) {
        ...
        case "informationCollection.updated":
            return runSnapshotSchema.parse({
                ...current,
                informationCollection: runInformationCollectionSchema.parse(parsed.informationCollection),
            });
        case "artifact.ready":
            return runSnapshotSchema.parse({
                ...current,
...
export function applyBridgeEventsToRunSnapshot(snapshot, events) {
    return events.reduce((current, event) => applyBridgeEventToRunSnapshot(current, event), snapshot);
}
export function createInformationCollectionPrompt(run) {
    return [
        "请问你需要我提供什么信息给你。",
        `当前任务标题：${run.title}`,
        `目标路径：${run.targetPath}`,
...
function countSatisfiedRequiredSlots(slots) {
    return slots.filter((slot) => slot.required && slot.status === "satisfied").length;
}
function countMissingRequiredSlots(slots) {
    return slots.filter((slot) => slot.required && slot.status !== "satisfied").length;
}
...
function isAttachmentSatisfiedSlot(slot) {
    return slot.type === "file" || slot.type === "directory";
}
function isTextSatisfiedSlot(slot) {
    return !isAttachmentSatisfiedSlot(slot);
}
function normalizeAnswerPreview(slot, valueText) {
    if (slot.secret) {
        return null;
    }
...
function buildStructuredAnswerId(sourceMessageId, slotKey, kind, index) {
    const safeSlotKey = slotKey.replace(/[^a-zA-Z0-9_-]+/g, "_");
    return `ica_${sourceMessageId}_${safeSlotKey}_${kind}_${index + 1}`;
}
function buildManualReviewAnswerId(answerId, at) {
...
function isEffectiveStructuredAnswer(answer) {
    return answer.reviewStatus !== "rejected" && answer.reviewStatus !== "superseded";
}
function countAnswersByReviewStatus(answers, reviewStatus) {
    return answers.filter((answer) => answer.reviewStatus === reviewStatus).length;
}
function collectStructuredAnswers(collection, options) {
    if (!options.sourceMessageId) {
        return [];
    }
    const slotByKey = new Map(collection.slots.map((slot) => [slot.key, slot]));
    const answers = [];
    let textIndex = 0;
    ...
    for (const slotValue of options.slotValues) {
        ...
        answers.push(runInformationCollectionAnswerSchema.parse({
            answerId: buildStructuredAnswerId(options.sourceMessageId, slot.key, "text", textIndex++),
            slotKey: slot.key,
            slotType: slot.type,
            kind: "text",
            source: "user-message",
    ...
    for (const attachment of options.attachments) {
        ...
        answers.push(runInformationCollectionAnswerSchema.parse({
            answerId: buildStructuredAnswerId(options.sourceMessageId, slot.key, "attachment", attachmentIndex++),
            slotKey: slot.key,
            slotType: slot.type,
            kind: "attachment",
            source: "user-message",
...
function inferSlotValuesFromMessage(collection, text, attachments) {
    ...
    const eligibleSlots = collection.slots.filter((slot) => slot.required &&
...
function rebuildInformationCollection(collection, options = {}) {
    ...
    const effectiveAnswersBySlot = answers.reduce((accumulator, answer) => {
    ...
    const slots = current.slots.map((slot) => {
        ...
        const effectiveAttachmentAnswers = effectiveAnswers.filter((answer) => answer.kind === "attachment");
        ...
        const satisfied = (canSatisfyWithAttachment && effectiveAttachmentAnswers.length > 0) ||
        ...
        return runInformationCollectionSlotSchema.parse({
            ...
            lastAnswerText: latestTextAnswer?.valueText != null
    ...
    return runInformationCollectionSchema.parse({
        ...
        lastUpdatedAt: options.lastUpdatedAt === undefined ? current.lastUpdatedAt : options.lastUpdatedAt,
        slots,
        answers,
    });
}
export function createRunInformationCollection(params) {
    const slots = (params.slots ?? []).map((slot) => runInformationCollectionSlotSchema.parse({
        ...slot,
    ...
    const requiredCount = slots.filter((slot) => slot.required).length;
    return rebuildInformationCollection(runInformationCollectionSchema.parse({
...
export function resolveInformationCollectionSlotValues(collection, options) {
    const current = runInformationCollectionSchema.parse(collection);
...
export function applyUserMessageToInformationCollection(collection, options) {
    const current = runInformationCollectionSchema.parse(collection);
    const attachments = options.attachments ?? [];
...
export function reviewInformationCollectionAnswer(collection, options) {
    const current = runInformationCollectionSchema.parse(collection);
    ...
    if (options.decision === "approve") {
        ...
        if (currentAnswer.kind === "text") {
            ...
            if (!replacementValueText) {
                throw new Error(`Text answer revisions require replacementValueText: ${options.answerId}`);
            }
            ...
            if (!replacementAttachmentPath) {
                throw new Error(`Attachment answer revisions require replacementAttachmentPath: ${options.answerId}`);
            }
            const revisedAnswer = runInformationCollectionAnswerSchema.parse({
                ...currentAnswer,
                answerId: buildManualReviewAnswerId(currentAnswer.answerId, options.at),
