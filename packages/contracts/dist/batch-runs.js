import { z } from "zod";
import { entrySurfaceSchema, isoDatetimeSchema, runIdSchema, sessionVersionIdSchema, taskVersionIdSchema, userIdSchema, workspaceIdSchema, } from "./common.js";
import { localizedTextSchema, serviceIdSchema, serviceLaunchTemplateResolutionSourceSchema, workshopIdSchema, workspaceContextKeySchema, } from "./catalog.js";
...
function requireBatchWorkspaceScope(schema) {
    return schema.refine((value) => Boolean(value.workspaceId || value.workspaceContextKey), "workspaceId or workspaceContextKey is required");
}
export const createBatchRunInputSchema = requireBatchWorkspaceScope(createBatchRunInputBaseSchema);
export const estimateBatchRunInputSchema = requireBatchWorkspaceScope(createBatchRunInputBaseSchema.omit({
    autoStart: true,
}));
export const importBatchRunFileInputSchema = z
    ...
    .refine((value) => Boolean(value.workspaceId || value.workspaceContextKey), "workspaceId or workspaceContextKey is required");
...
export const batchRunRetryInputSchema = z
    ...
    .refine((value) => value.onlyFailed || Boolean(value.itemIds && value.itemIds.length > 0), "itemIds are required when onlyFailed is false");
