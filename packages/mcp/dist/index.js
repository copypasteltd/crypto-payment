...
import { mcpBindingSchema, mcpNetworkPolicySchema, } from "@lingban/contracts";
export function slugifyMcpKey(value) {
    return value
        .replace(/^mcp\./, "")
...
export function buildMcpAuthDescriptor(mount) {
    if (!mount) {
        return {
            authMode: null,
            authRef: null,
...
export function buildRuntimeMcpBinding(input) {
    ...
    return mcpBindingSchema.parse({
        bindingId: input.persistedBinding?.bindingId ?? `mbd_${slugifyMcpKey(input.entry.mcpId)}`,
        ...
        networkPolicyRef: input.persistedBinding?.networkPolicyRef ??
            input.entry.defaultNetworkPolicyRef ??
            null,
        approvalRequired: input.persistedBinding?.approvalRequired ?? input.entry.approvalRequired,
    });
}
...
export function matchesMcpNetworkHostPattern(hostname, pattern) {
...
export function assertMcpNetworkPolicyShape(policy) {
    ...
    if (parsed.requireTls && parsed.allowedProtocols.some((item) => insecureProtocols.has(item))) {
        throw new Error(`Network policy ${parsed.policyRef} requires TLS and cannot allow insecure protocols`);
    }
    for (const pattern of parsed.allowedHostPatterns) {
        if (!isValidHostnamePattern(normalizeHostnamePattern(pattern))) {
            throw new Error(`Network policy ${parsed.policyRef} contains an invalid host pattern: ${pattern}`);
        }
    }
    for (const prefix of parsed.allowedPathPrefixes) {
        if (!prefix.startsWith("/")) {
            throw new Error(`Network policy ${parsed.policyRef} must use absolute path prefixes`);
        }
    }
    return parsed;
}
function normalizeFilesystemPathForComparison(value) {
    ...
    const caseInsensitive = path.win32.isAbsolute(value) || /^[a-zA-Z]:\//.test(normalized) || normalized.startsWith("//");
    const next = normalized.length > 1 ? normalized.replace(/\/+$/g, "") : normalized;
    return caseInsensitive ? next.toLowerCase() : next;
}
function isAbsoluteFilesystemPath(value) {
    return path.posix.isAbsolute(value) || path.win32.isAbsolute(value);
}
function matchesFilesystemPathPrefix(targetPath, prefix) {
...
export function evaluateMcpStdioPathAllowlist(input) {
    ...
    if (!input.allowedPathPrefixes.some((prefix) => matchesFilesystemPathPrefix(input.targetPath, prefix))) {
        return {
            allowed: false,
...
export async function evaluateMcpStdioIntegrity(input) {
    ...
    const statImpl = input.statImpl ??
        (async (targetPath) => await fs.stat(targetPath));
    const readFileImpl = input.readFileImpl ??
        (async (targetPath) => await fs.readFile(targetPath));
    let stat;
    try {
...
export function evaluateMcpNetworkPolicy(input) {
    ...
    if (!policy.allowedHostPatterns.some((pattern) => matchesMcpNetworkHostPattern(hostname, pattern))) {
        return {
            allowed: false,
    ...
    if (policy.allowedPorts.length > 0) {
        const resolvedPort = parsedUrl.port.length > 0
    ...
    if (policy.allowedPathPrefixes.length > 0 &&
        !policy.allowedPathPrefixes.some((prefix) => matchesPathPrefix(parsedUrl.pathname || "/", prefix))) {
        return {
            allowed: false,
...
export function assertRuntimeMcpBindings(input) {
    ...
    const summary = issues
        .map((issue) => `${issue.mcpId} (${issue.bindingId}) [${issue.reasonCode}]: ${issue.message}`)
        .join("; ");
