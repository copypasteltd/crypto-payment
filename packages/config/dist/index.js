...
const credentialLifecycleCallbackOauthClientAuthenticationSchema = z.discriminatedUnion("method", [
    z.object({
...
const credentialLifecycleCallbacksSchema = z.record(z.string(), credentialLifecycleCallbackConfigSchema);
function resolveStorageDriver(explicit, fallback) {
...
function readCommandArgs(env, key) {
    const value = readTrimmed(env, key);
    if (!value) {
        return undefined;
    }
...
function readStringList(env, key, fallback) {
    const value = readTrimmed(env, key);
    if (!value) {
        return [...fallback];
    }
...
function readJsonObject(env, key) {
    const value = readTrimmed(env, key);
    if (!value) {
        return undefined;
    }
...
function resolveFromCwd(value, fallbackSegments) {
...
export function loadApiRuntimeConfig(env = process.env) {
    ...
    const objectStorageDriver = objectStorageDriverSchema.parse(readTrimmed(env, "LINGBAN_OBJECT_STORAGE_DRIVER") ?? "filesystem");
    ...
    const runEventsStore = storageDriverSchema.parse(readTrimmed(env, "LINGBAN_RUN_EVENTS_STORE") ?? "file");
    const legacyAuxStoreFallback = runsStore === "postgres" || runEventsStore === "postgres" ? "postgres" : "file";
    ...
    const credentialBrokerProvider = credentialBrokerProviderSchema.parse(readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_PROVIDER") ?? "local-envelope");
    const sessionPackSignatureEnabled = readBoolean(env, "LINGBAN_SESSION_PACK_SIGNATURE_ENABLED", false);
    const sessionPackSignatureRequireForImports = readBoolean(env, "LINGBAN_SESSION_PACK_SIGNATURE_REQUIRE_FOR_IMPORTS", false);
    const sessionPackSignatureAlgorithm = sessionPackSignatureAlgorithmSchema.parse(readTrimmed(env, "LINGBAN_SESSION_PACK_SIGNATURE_ALGORITHM") ?? "hmac-sha256");
    const sessionPackSignatureKeyId = readTrimmed(env, "LINGBAN_SESSION_PACK_SIGNATURE_KEY_ID");
    const sessionPackSignatureHmacSecret = readTrimmed(env, "LINGBAN_SESSION_PACK_SIGNATURE_HMAC_SECRET");
    ...
    const sessionPackSignatureEd25519PrivateKeyPem = readTrimmed(env, "LINGBAN_SESSION_PACK_SIGNATURE_ED25519_PRIVATE_KEY_PEM");
    const sessionPackSignatureEd25519PublicKeyPem = readTrimmed(env, "LINGBAN_SESSION_PACK_SIGNATURE_ED25519_PUBLIC_KEY_PEM");
    ...
    const sessionPackSignatureDistributionTargets = z
        ...
        .parse((() => {
        const raw = readTrimmed(env, "LINGBAN_SESSION_PACK_SIGNATURE_DISTRIBUTION_TARGETS_JSON");
        if (!raw) {
            return [];
        }
        return JSON.parse(raw);
    })());
    const sessionPackSignatureDistributionStaleAfterMs = readPositiveInteger(env, "LINGBAN_SESSION_PACK_SIGNATURE_DISTRIBUTION_STALE_AFTER_MS", 86_400_000);
    const sessionPackGovernancePolicyRequireApprovedRedactionReviewForRedactedExport = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_REQUIRE_APPROVED_REDACTION_REVIEW_FOR_REDACTED_EXPORT", true);
    const sessionPackGovernancePolicyRequireApprovedRedactionReviewForPublish = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_REQUIRE_APPROVED_REDACTION_REVIEW_FOR_PUBLISH", true);
    const sessionPackGovernancePolicyRequireSuccessfulRedactionPreviewForRedactedExport = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_REQUIRE_SUCCESSFUL_REDACTION_PREVIEW_FOR_REDACTED_EXPORT", true);
    const sessionPackGovernancePolicyRequireSuccessfulRedactionPreviewForPublish = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_REQUIRE_SUCCESSFUL_REDACTION_PREVIEW_FOR_PUBLISH", true);
    const sessionPackGovernancePolicyRequireCompleteSecretCoverageForRedactedExport = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_REQUIRE_COMPLETE_SECRET_COVERAGE_FOR_REDACTED_EXPORT", true);
    const sessionPackGovernancePolicyRequireCompleteSecretCoverageForPublish = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_REQUIRE_COMPLETE_SECRET_COVERAGE_FOR_PUBLISH", true);
    const sessionPackGovernancePolicyRequireVerifiedSignatureForInherit = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_REQUIRE_VERIFIED_SIGNATURE_FOR_INHERIT", true);
    const sessionPackGovernancePolicyRequireVerifiedSignatureForPublish = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_REQUIRE_VERIFIED_SIGNATURE_FOR_PUBLISH", true);
    const sessionPackGovernancePolicyRequireVerifiedSignatureForRollback = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_REQUIRE_VERIFIED_SIGNATURE_FOR_ROLLBACK", true);
    const sessionPackGovernancePolicyRequireSignatureKeyAcceptedForPublish = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_REQUIRE_SIGNATURE_KEY_ACCEPTED_FOR_PUBLISH", true);
    const sessionPackGovernancePolicyRequireSignatureKeyAcceptedForRollback = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_REQUIRE_SIGNATURE_KEY_ACCEPTED_FOR_ROLLBACK", true);
    const sessionPackGovernancePolicyRequireSignatureKeyDistributionForPublish = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_REQUIRE_SIGNATURE_KEY_DISTRIBUTION_FOR_PUBLISH", true);
    const sessionPackGovernancePolicyRequireSignatureKeyDistributionForRollback = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_REQUIRE_SIGNATURE_KEY_DISTRIBUTION_FOR_ROLLBACK", true);
    const sessionPackGovernancePolicyWarnOnActiveSigningKeyDistributionDrift = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_WARN_ON_ACTIVE_SIGNING_KEY_DISTRIBUTION_DRIFT", true);
    const sessionPackGovernancePolicyWarnOnUnpublishWithLiveConsumers = readBoolean(env, "LINGBAN_SESSION_PACK_POLICY_WARN_ON_UNPUBLISH_WITH_LIVE_CONSUMERS", true);
    const credentialBrokerAwsRegion = readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_AWS_REGION") ??
        readTrimmed(env, "AWS_REGION") ??
        readTrimmed(env, "AWS_DEFAULT_REGION");
    const credentialBrokerAwsKmsKeyId = readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_AWS_KMS_KEY_ID");
    const credentialBrokerAwsEndpoint = readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_AWS_ENDPOINT");
    const credentialBrokerAwsAccessKeyId = readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_AWS_ACCESS_KEY_ID") ??
        readTrimmed(env, "AWS_ACCESS_KEY_ID");
    const credentialBrokerAwsSecretAccessKey = readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_AWS_SECRET_ACCESS_KEY") ??
        readTrimmed(env, "AWS_SECRET_ACCESS_KEY");
    const credentialBrokerAwsSessionToken = readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_AWS_SESSION_TOKEN") ??
        readTrimmed(env, "AWS_SESSION_TOKEN");
    const credentialBrokerActiveKeyId = readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_ACTIVE_KEY_ID") ??
        (() => {
    ...
    const credentialBrokerKeys = credentialBrokerProvider === "local-envelope"
        ...
        : {};
    const credentialBrokerUsesDefaultKey = credentialBrokerProvider === "local-envelope" &&
        !readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_KEYS_JSON") &&
        !readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_MASTER_KEY");
    const credentialBrokerVaultBaseUrl = readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_VAULT_BASE_URL");
    const credentialBrokerVaultToken = readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_VAULT_TOKEN");
    const credentialBrokerVaultNamespace = readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_VAULT_NAMESPACE");
    const credentialBrokerVaultTransitMount = readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_VAULT_TRANSIT_MOUNT") ?? "transit";
    const credentialBrokerVaultKeyType = readTrimmed(env, "LINGBAN_CREDENTIAL_BROKER_VAULT_KEY_TYPE") ?? "aes256-gcm96";
    ...
    if (credentialBrokerProvider === "vault-transit-http" && !credentialBrokerVaultBaseUrl) {
        throw new Error("LINGBAN_CREDENTIAL_BROKER_VAULT_BASE_URL is required when provider=vault-transit-http");
    }
    if (credentialBrokerProvider === "vault-transit-http" && !credentialBrokerVaultToken) {
        throw new Error("LINGBAN_CREDENTIAL_BROKER_VAULT_TOKEN is required when provider=vault-transit-http");
    }
    if (credentialBrokerProvider === "aws-kms-envelope" && !credentialBrokerAwsRegion) {
        throw new Error("LINGBAN_CREDENTIAL_BROKER_AWS_REGION or AWS_REGION is required when provider=aws-kms-envelope");
    }
    if (credentialBrokerProvider === "aws-kms-envelope" && !credentialBrokerAwsKmsKeyId) {
        throw new Error("LINGBAN_CREDENTIAL_BROKER_AWS_KMS_KEY_ID is required when provider=aws-kms-envelope");
    }
    if (credentialBrokerProvider === "aws-kms-envelope" &&
        credentialBrokerAwsAccessKeyId &&
        !credentialBrokerAwsSecretAccessKey) {
        throw new Error("LINGBAN_CREDENTIAL_BROKER_AWS_SECRET_ACCESS_KEY is required when AWS access key id is configured");
    }
    if (credentialBrokerProvider === "aws-kms-envelope" &&
        !credentialBrokerAwsAccessKeyId &&
        credentialBrokerAwsSecretAccessKey) {
        throw new Error("LINGBAN_CREDENTIAL_BROKER_AWS_ACCESS_KEY_ID is required when AWS secret access key is configured");
    }
    const fileScanMode = fileScanModeSchema.parse(readTrimmed(env, "LINGBAN_FILE_SCAN_MODE") ?? "builtin");
    ...
    if ((sessionPackSignatureEnabled || sessionPackSignatureRequireForImports) &&
        sessionPackSignatureAlgorithm === "hmac-sha256" &&
        Object.keys(sessionPackSignatureHmacKeysByKeyId).length === 0) {
        throw new Error("LINGBAN_SESSION_PACK_SIGNATURE_HMAC_SECRET or LINGBAN_SESSION_PACK_SIGNATURE_HMAC_KEYS_JSON is required when session-pack signature algorithm=hmac-sha256");
    }
    if (sessionPackSignatureEnabled &&
        sessionPackSignatureAlgorithm === "ed25519" &&
        !sessionPackSignatureEd25519PrivateKeyPem) {
        throw new Error("LINGBAN_SESSION_PACK_SIGNATURE_ED25519_PRIVATE_KEY_PEM is required when session-pack signature signing is enabled with algorithm=ed25519");
    }
    if (sessionPackSignatureRequireForImports &&
        sessionPackSignatureAlgorithm === "ed25519" &&
        !sessionPackSignatureEd25519PrivateKeyPem &&
        !sessionPackSignatureEd25519PublicKeyPem &&
        Object.keys(sessionPackSignatureEd25519PublicKeysByKeyId).length === 0) {
        throw new Error("LINGBAN_SESSION_PACK_SIGNATURE_ED25519_PUBLIC_KEY_PEM, LINGBAN_SESSION_PACK_SIGNATURE_ED25519_PUBLIC_KEYS_JSON, or LINGBAN_SESSION_PACK_SIGNATURE_ED25519_PRIVATE_KEY_PEM is required when import verification uses algorithm=ed25519");
    }
    return {
        host,
        port,
        ...
        catalogStore,
        workshopCatalogStore: resolveStorageDriver(readTrimmed(env, "LINGBAN_WORKSHOP_CATALOG_STORE"), catalogStore),
        creatorStore,
        runsStore,
        batchRunsStore: resolveStorageDriver(readTrimmed(env, "LINGBAN_BATCH_RUNS_STORE"), runsStore),
        runEventsStore,
        internalCallbacksStore: resolveStorageDriver(readTrimmed(env, "LINGBAN_INTERNAL_CALLBACKS_STORE"), runEventsStore),
        authStore,
        uploadsStore: resolveStorageDriver(readTrimmed(env, "LINGBAN_UPLOADS_STORE"), legacyAuxStoreFallback),
        runFilesStore: resolveStorageDriver(readTrimmed(env, "LINGBAN_RUN_FILES_STORE"), runsStore),
        bridgeRegistryStore: resolveStorageDriver(readTrimmed(env, "LINGBAN_BRIDGE_REGISTRY_STORE"), legacyAuxStoreFallback),
        credentialsStore: resolveStorageDriver(readTrimmed(env, "LINGBAN_CREDENTIALS_STORE"), legacyAuxStoreFallback),
        ...
        notificationsStore: resolveStorageDriver(readTrimmed(env, "LINGBAN_NOTIFICATIONS_STORE"), authStore),
        favoritesStore: resolveStorageDriver(readTrimmed(env, "LINGBAN_FAVORITES_STORE"), authStore),
        recentStore: resolveStorageDriver(readTrimmed(env, "LINGBAN_RECENT_STORE"), authStore),
        searchStore: resolveStorageDriver(readTrimmed(env, "LINGBAN_SEARCH_STORE"), authStore),
        sessionArchivesStore: resolveStorageDriver(readTrimmed(env, "LINGBAN_SESSION_ARCHIVES_STORE"), runsStore),
        ...
        authRefreshTokenTtlSeconds: readPositiveInteger(env, "LINGBAN_AUTH_REFRESH_TTL_SECONDS", 60 * 60 * 24 * 30),
        objectStorageDriver,
        ...
        objectStorageForcePathStyle: readBoolean(env, "LINGBAN_OBJECT_STORAGE_FORCE_PATH_STYLE", false),
        objectStorageSignedUrlTtlSeconds: readPositiveInteger(env, "LINGBAN_OBJECT_STORAGE_SIGNED_URL_TTL_SECONDS", 600),
        uploadMaxBytes: readPositiveInteger(env, "LINGBAN_UPLOAD_MAX_BYTES", 52_428_800),
        downloadTicketTtlSeconds: readPositiveInteger(env, "LINGBAN_DOWNLOAD_TICKET_TTL_SECONDS", 600),
        storageRetentionSweepIntervalMs: readPositiveInteger(env, "LINGBAN_STORAGE_RETENTION_SWEEP_INTERVAL_MS", 5 * 60 * 1000),
        downloadTicketRetentionSeconds: readPositiveInteger(env, "LINGBAN_DOWNLOAD_TICKET_RETENTION_SECONDS", 60 * 60),
        unattachedUploadTtlSeconds: readPositiveInteger(env, "LINGBAN_UNATTACHED_UPLOAD_TTL_SECONDS", 60 * 60 * 24),
        expiredUploadRetentionSeconds: readPositiveInteger(env, "LINGBAN_EXPIRED_UPLOAD_RETENTION_SECONDS", 60 * 60 * 24 * 7),
        runFileLifecycleSweepIntervalMs: readPositiveInteger(env, "LINGBAN_RUN_FILE_LIFECYCLE_SWEEP_INTERVAL_MS", 10 * 60 * 1000),
        runFileArchiveHotRetentionSeconds: readPositiveInteger(env, "LINGBAN_RUN_FILE_ARCHIVE_HOT_RETENTION_SECONDS", 60 * 60 * 24),
        ...
        fileScanMode,
        fileScanErrorPolicy: fileScanErrorPolicySchema.parse(readTrimmed(env, "LINGBAN_FILE_SCAN_ERROR_POLICY") ?? "block"),
        fileScanTimeoutMs: readPositiveInteger(env, "LINGBAN_FILE_SCAN_TIMEOUT_MS", 10_000),
        ...
        fileScanBlockedMimePrefixes: readStringList(env, "LINGBAN_FILE_SCAN_BLOCKED_MIME_PREFIXES", [
            "application/x-bat",
        ...
        fileScanBlockMacroEnabledOffice: readBoolean(env, "LINGBAN_FILE_SCAN_BLOCK_MACRO_OFFICE", true),
        fileScanClamavHost,
        fileScanClamavPort: readPositiveInteger(env, "LINGBAN_FILE_SCAN_CLAMAV_PORT", 3310),
        bridgeRegistrationStaleAfterMs: readPositiveInteger(env, "LINGBAN_BRIDGE_REGISTRATION_STALE_AFTER_MS", 20_000),
        bridgeRegistrationSweepIntervalMs: readPositiveInteger(env, "LINGBAN_BRIDGE_REGISTRATION_SWEEP_INTERVAL_MS", 5_000),
        workerOpsBaseUrl: readTrimmed(env, "LINGBAN_WORKER_OPS_BASE_URL"),
        workerOpsToken: readTrimmed(env, "LINGBAN_WORKER_OPS_TOKEN"),
        workerOpsProbeTimeoutMs: readPositiveInteger(env, "LINGBAN_WORKER_OPS_PROBE_TIMEOUT_MS", 1_000),
        bridgeControlProbeTimeoutMs: readPositiveInteger(env, "LINGBAN_BRIDGE_CONTROL_PROBE_TIMEOUT_MS", 1_000),
        mcpProbeTimeoutMs: readPositiveInteger(env, "LINGBAN_MCP_PROBE_TIMEOUT_MS", 3_000),
        mcpStdioAllowedPathPrefixes: readStringList(env, "LINGBAN_MCP_STDIO_ALLOWED_PATH_PREFIXES", []),
        mcpStdioRequireRefSha256: readBoolean(env, "LINGBAN_MCP_STDIO_REQUIRE_REF_SHA256", true),
        credentialBrokerProvider,
        credentialBrokerActiveKeyId,
        credentialBrokerKeys,
        credentialBrokerLeaseTtlSeconds: readPositiveInteger(env, "LINGBAN_CREDENTIAL_BROKER_LEASE_TTL_SECONDS", 300),
        credentialBrokerUsesDefaultKey,
        credentialBrokerRequestTimeoutMs: readPositiveInteger(env, "LINGBAN_CREDENTIAL_BROKER_REQUEST_TIMEOUT_MS", 5_000),
        ...
        credentialLifecycleSweepIntervalMs: readPositiveInteger(env, "LINGBAN_CREDENTIAL_LIFECYCLE_SWEEP_INTERVAL_MS", 60_000),
        credentialLifecycleCallbacks,
        credentialLifecycleCallbackSweepIntervalMs: readPositiveInteger(env, "LINGBAN_CREDENTIAL_LIFECYCLE_CALLBACK_SWEEP_INTERVAL_MS", 60_000),
        credentialLifecycleCallbackMaxAttempts: readPositiveInteger(env, "LINGBAN_CREDENTIAL_LIFECYCLE_CALLBACK_MAX_ATTEMPTS", 8),
        credentialLifecycleCallbackBackoffMs: readPositiveInteger(env, "LINGBAN_CREDENTIAL_LIFECYCLE_CALLBACK_BACKOFF_MS", 5_000),
...
export function loadWorkerRuntimeConfig(env = process.env) {
    ...
    const inferredRunnerUid = explicitRunnerUid ?? (currentProcessUid != null && currentProcessUid > 0 ? currentProcessUid : undefined);
    const inferredRunnerGid = explicitRunnerGid ?? (currentProcessGid != null && currentProcessGid > 0 ? currentProcessGid : undefined);
    if (runnerDropRootRaw &&
        runnerDropRootRequested &&
        (inferredRunnerUid == null || inferredRunnerGid == null)) {
        throw new Error("LINGBAN_RUNNER_DROP_ROOT_ENABLED requires a resolvable runtime uid/gid via LINGBAN_RUNNER_UID/GID or the current worker process identity");
    }
    const runnerDropRootEnabled = runnerDropRootRequested && inferredRunnerUid != null && inferredRunnerGid != null;
    return {
        ...
        runtimeEgressProxyEnabled: readBoolean(env, "LINGBAN_RUNTIME_EGRESS_PROXY_ENABLED", false),
        runtimeEgressAllowedBaseUrls: readStringList(env, "LINGBAN_RUNTIME_EGRESS_ALLOWED_BASE_URLS", []),
        runtimeEgressNoProxyHosts: readStringList(env, "LINGBAN_RUNTIME_EGRESS_NO_PROXY_HOSTS", ["127.0.0.1", "localhost", "host.docker.internal"]),
        runtimeEgressFirewallEnabled: readBoolean(env, "LINGBAN_RUNTIME_EGRESS_FIREWALL_ENABLED", false),
        runtimeEgressFirewallAllowDns: readBoolean(env, "LINGBAN_RUNTIME_EGRESS_FIREWALL_ALLOW_DNS", true),
        internalAuthToken: readTrimmed(env, "LINGBAN_INTERNAL_AUTH_TOKEN"),
        ...
        opsProbeTimeoutMs: readPositiveInteger(env, "LINGBAN_WORKER_OPS_PROBE_TIMEOUT_MS", 1_000),
        runtimeDispatchMode: runtimeDispatchModeSchema.parse(readTrimmed(env, "LINGBAN_RUNTIME_DISPATCH_MODE") ?? "embedded"),
        runtimeLaunchMode: runtimeLaunchModeSchema.parse(readTrimmed(env, "LINGBAN_RUNTIME_LAUNCH_MODE") ?? "docker"),
        maxConcurrentRuns: readPositiveInteger(env, "LINGBAN_RUNTIME_MAX_CONCURRENT_RUNS", 2),
        orphanRecoveryGraceMs: readPositiveInteger(env, "LINGBAN_RUNTIME_ORPHAN_GRACE_MS", 15_000),
        terminalWorkspaceTtlMs: readPositiveInteger(env, "LINGBAN_TERMINAL_WORKSPACE_TTL_MS", 15 * 60 * 1000),
        ...
        runCleanupQueueName: readTrimmed(env, "LINGBAN_RUNTIME_CLEANUP_QUEUE_NAME") ?? "run.cleanup",
        runStartDlqQueueName: readTrimmed(env, "LINGBAN_RUNTIME_START_DLQ_QUEUE_NAME") ?? "run.start.dlq",
        runCleanupDlqQueueName: readTrimmed(env, "LINGBAN_RUNTIME_CLEANUP_DLQ_QUEUE_NAME") ?? "run.cleanup.dlq",
        runStartJobAttempts: readPositiveInteger(env, "LINGBAN_RUNTIME_START_JOB_ATTEMPTS", 3),
        runStartJobBackoffMs: readPositiveInteger(env, "LINGBAN_RUNTIME_START_JOB_BACKOFF_MS", 2_000),
        runCleanupJobAttempts: readPositiveInteger(env, "LINGBAN_RUNTIME_CLEANUP_JOB_ATTEMPTS", 5),
        runCleanupJobBackoffMs: readPositiveInteger(env, "LINGBAN_RUNTIME_CLEANUP_JOB_BACKOFF_MS", 5_000),
        dockerBin: readTrimmed(env, "LINGBAN_DOCKER_BIN") ?? "docker",
        runtimeStartupTimeoutMs: readPositiveInteger(env, "LINGBAN_RUNTIME_STARTUP_TIMEOUT_MS", 15_000),
...
export function loadBridgeCliRuntimeConfig(env = process.env, argv = process.argv.slice(2)) {
    ...
    return {
        ...
        registrationRefreshMs: readPositiveInteger(env, "LINGBAN_BRIDGE_REGISTRATION_REFRESH_MS", 5_000),
        codexRestartMaxAttempts: readNonNegativeInteger(env, "LINGBAN_BRIDGE_CODEX_RESTART_MAX_ATTEMPTS", 2),
        codexRestartBackoffMs: readNonNegativeInteger(env, "LINGBAN_BRIDGE_CODEX_RESTART_BACKOFF_MS", 1_000),
        codexRestartResetWindowMs: readNonNegativeInteger(env, "LINGBAN_BRIDGE_CODEX_RESTART_RESET_WINDOW_MS", 30_000),
