import { runFileRecordSchema } from "@lingban/contracts";
function sortFiles(files) {
    return [...files].sort((left, right) => left.path.localeCompare(right.path));
}
export class CachedRunFilesIndexRepository {
    #filesByRun = new Map();
    #writeChains = new Map();
    async init() {
    ...
    #setRecord(record) {
        const runBucket = this.#filesByRun.get(record.runId) ?? new Map();
        runBucket.set(record.path, record);
        this.#filesByRun.set(record.runId, runBucket);
    }
    #replaceRun(runId, records) {
        const runBucket = new Map();
    ...
    async replaceRunFiles(runId, files) {
    ...
    async upsertRunFile(file) {
        const parsed = runFileRecordSchema.parse(file);
        this.#setRecord(parsed);
        await this.#enqueuePersist(parsed.runId, () => this.persistRun(parsed.runId, this.listRunFiles(parsed.runId)));
        return parsed;
    }
    async clear() {
...
//# sourceMappingURL=run-files-repository.js.map