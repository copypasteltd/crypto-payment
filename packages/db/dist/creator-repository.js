import { creatorAuditExportIdSchema, creatorAuditExportRecordSchema, creatorGovernanceDynamicSectionSchema, creatorPackageDetailSchema, creatorPackageIdSchema, creatorReleaseActivationIdSchema, creatorReleaseActivationSchema, creatorReleaseGateIdSchema, creatorReleaseGateSchema, creatorReleaseIdSchema, creatorReleaseSummarySchema, creatorReplayIdSchema, creatorReplaySummarySchema, } from "@lingban/contracts";
import { z } from "zod";
...
export class CachedCreatorRepository {
    #initialized = false;
    #state = creatorStateSchema.parse({
        packages: [],
        releases: [],
        replays: [],
        releaseGates: [],
    ...
    listReleasesByPackage(packageId) {
    ...
    listReplaysByPackage(packageId) {
    ...
    listReleaseGatesByRelease(releaseId) {
    ...
    async saveReleaseActivation(activation) {
        await this.init();
...
export class PostgresCreatorRepository extends CachedCreatorRepository {
    ...
    async loadState() {
        ...
        const [packages, releases, replays, releaseGates, activations, auditExports] = await Promise.all([
            queryable.query("SELECT package_json FROM lingban_creator_packages ORDER BY package_id ASC"),
            queryable.query("SELECT release_json FROM lingban_creator_releases ORDER BY release_id ASC"),
            queryable.query("SELECT replay_json FROM lingban_creator_replays ORDER BY replay_id ASC"),
            queryable.query("SELECT gate_json FROM lingban_creator_release_gates ORDER BY gate_id ASC"),
            queryable.query("SELECT activation_json FROM lingban_creator_release_activations ORDER BY activation_id ASC"),
            queryable.query("SELECT export_json FROM lingban_creator_audit_exports ORDER BY created_at DESC, export_id ASC"),
        ]);
